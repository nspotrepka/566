...
from models.cyclegan.generator import VAEGenerator
import warnings
...
class CycleVAEGAN(pl.LightningModule):
    def __init__(self, train_loader, val_loader, data_size,
                 in_channels, out_channels, g_filters=64, d_filters=64,
                 z_size=256, hidden_layers=2, hidden_size=384,
                 learning_rate=0.0002, beta_1=0.5, beta_2=0.999,
                 init_type='xavier', init_scale=0.02, pool_size_a=50,
                 pool_size_b=50, lambda_a=10.0, lambda_b=10.0, lambda_id=0.0,
                 lambda_g=1.0, lambda_d=1.0, lambda_kl=1.0, lambda_recon=1.0):
    ...
    def forward(self, input):
        ...
        self.fake_a, self.z, self.mu, self.logvar = self.gen_b_to_a(self.real_b)
        self.fake_b, self.z, self.mu, self.logvar = self.gen_a_to_b(self.real_a)
        self.cycle_a, _, _, _ = self.gen_b_to_a(self.fake_b)
        self.cycle_b, _, _, _ = self.gen_a_to_b(self.fake_a)
    ...
    def backward_g(self):
        if self.lambda_id > 0.0:
            self.id_a, _, _, _ = self.gen_b_to_a(self.real_a)
            self.id_b, _, _, _ = self.gen_a_to_b(self.real_b)
        ...
        self.loss_kl = self.kl_div(self.mu, self.logvar)
        self.loss_kl *= self.lambda_kl
        ...
        self.loss_g = self.loss_kl + self.loss_gan + self.loss_cycle
